...
// Function for the specular term (specular BRDF)
vec3 specularBRDF(vec3 F, float NDF, float G, vec3 N, vec3 viewDirection, vec3 Lh)
{
    // Adjust the denominator for energy conservation
    float denominator = max(0.001, dot(N, viewDirection)) * max(0.001, dot(N, Lh)) + 0.001;

    return (F * NDF * G) / denominator;
}

// Function for the diffuse term (diffuse BRDF)
vec3 diffuseBRDF(vec3 albedo)
{
    // Adjust the scaling factor for diffuse reflection
    float scale = 1.0 / (PI * 0.5);
...
void main()
{
    ...
    for (int i=0; i<NumLights; ++i)
    {
        ...
        */
        ...
        // Calculate the Normal Distribution Function (GGX NDF) using roughness
        float alpha = roughness * roughness;
        float NDF = alpha * alpha / (PI * pow(max(0.001, dot(N, Lh)), 4.0) * (alpha * alpha + dot(N, Lh) * dot(N, Lh)) * (alpha * alpha + dot(N, Lh) * dot(N, Lh)));

        // Calculate the joint masking-shadowing function (Geometric Attenuation)
        float k = (roughness + 1.0) * (roughness + 1.0) / 8.0;
        float G = min(1.0, min(dot(N, viewDirection) / (dot(N, viewDirection) * (1.0 - k) + k), dot(N, Lh) / (dot(N, Lh) * (1.0 - k) + k)));

        // Calculate the specular BRDF
        vec3 specularTerm = specularBRDF(F, NDF, G, N, viewDirection, Lh);

        // Calculate the diffuse BRDF
        vec3 diffuseTerm = diffuseBRDF(albedo);

        // Combine the specular and diffuse terms, considering energy conservation
        directLighting += specularTerm + diffuseTerm;
