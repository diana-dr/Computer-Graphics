...
namespace cgCourse
{
	...
	bool GLExample::render()
	{
        ...
        const float fieldOfView = glm::radians(90.0f);
        const float nearPlane = 0.1f;
        const float farPlane = 50.0f;
        const float aspect = static_cast<float>(getFramebufferSize().x) / static_cast<float>(getFramebufferSize().y);

        glm::mat4 projectionMatrix = glm::perspective(fieldOfView, aspect, nearPlane, farPlane);
        glm::vec3 position = lightbox->getPosition();
